...
def multi_model_eval_loop(config : FoundationConfig, 
                        model, 
                        val_dataset, 
                        save_dir):
    ...
    numpy_path = os.path.join("predictions", "numpy_eff")
    if not os.path.exists(numpy_path):
        os.mkdir(numpy_path)
        os.chmod(numpy_path, 0o777)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(zip(val_dataloader)):
            ...
            building_prediction = building_pred.cpu().numpy()[0] # index so shape is (H,W) for buildings
            road_prediction = roadspeed_pred.cpu().numpy()[0] # index so we have (C,H,W)

            save_path = os.path.join(numpy_path, os.path.basename(current_image_filename.replace(".tif", ".npy")))
...
if __name__ == "__main__":
    ...
    multi_model_eval_loop(config, model, val_dataset, path)
